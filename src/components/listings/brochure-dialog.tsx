...
export function BrochureDialog({ property, children }: BrochureDialogProps) {
  ...
  const handlePrint = (e: React.MouseEvent) => {
    // We prevent the default dialog opening if we want it "automatic"
    // However, Shadcn Dialog handles its own state. 
    // To make it feel automatic, we'll trigger print immediately when the content is ready.
    // A better way is to wait for the dialog to open or use a hidden print ref.
    setTimeout(() => {
      window.print();
    }, 500);
  };
  ...
  return (
    <Dialog onOpenChange={(open) => { if(open) setTimeout(() => window.print(), 500); }}>
      <DialogTrigger asChild>
        {children}
      </DialogTrigger>
      <DialogContent className="max-w-[95vw] md:max-w-5xl p-0 bg-white border-none overflow-hidden rounded-none h-[90vh] flex flex-col z-[100] print:h-screen print:w-screen print:fixed print:inset-0 print:z-[9999]">
        ...
        <div className="flex-grow overflow-y-auto p-12 bg-muted/30 print:p-0 print:bg-white">
          {/* A4 Container */}
          <div id="printable-brochure" ref={brochureRef} className="w-full max-w-[800px] mx-auto bg-white shadow-2xl aspect-[1/1.414] flex flex-col print:shadow-none print:w-[210mm] print:h-[297mm]">
            ...
            <div className="flex-grow w-full flex overflow-hidden">
              {/* Left Column: Narrative & Technicals */}
              <div className="flex-grow p-10 flex flex-col gap-6 overflow-hidden">
                {/* Detail Shots Grid */}
                <div className="grid grid-cols-3 gap-2 aspect-[3/1.2] shrink-0">
                  {gallery.map((img, i) => (
                    <div key={i} className="relative w-full h-full bg-muted">
                ...
                <div className="space-y-3 flex-grow overflow-hidden">
                  ...
                  <p className="text-[9.5px] leading-relaxed text-black/50 font-serif italic text-justify line-clamp-[10]">
                    {description}
                  </p>
                </div>
              </div>

              {/* Right Column: High-Contrast Contact (Black) */}
              <div className="w-[35%] bg-black p-10 flex flex-col justify-between text-white shrink-0">
                ...
                <div className="space-y-6 flex flex-col items-center border-t border-white/10 pt-10">
...
function ContactDetail({ icon: Icon, text }: { icon: any, text: string }) {
  return (
    <div className="flex items-center gap-4 group">
      ...
      <span className="text-[8.5px] font-bold tracking-[0.2em] uppercase text-white/80 truncate">{text}</span>
    </div>
  );
}
